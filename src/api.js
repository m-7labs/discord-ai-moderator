...
const { _body, validationResult } = require('express-validator');
  ...
  const legacyLimiter = rateLimit({
    ...
    keyGenerator: (req) => {
      ...
      if (forwarded) {
        ...
        // Validate IP format
        // Use safer approach to validate IPs
        if (SecurityValidator.validateIpAddress(ip)) {
          return ip;
        }
      }
...
function getObjectDepth(obj, depth = 0) {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      // Use safer approach with explicit property access
...
function hasLargeArrays(obj, maxSize = 1000, visited = new WeakSet()) {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      // eslint-disable-next-line security/detect-object-injection
      const value = obj[key];
      if (hasLargeArrays(value, maxSize, visited)) {
        return true;
      }
    }
  }
