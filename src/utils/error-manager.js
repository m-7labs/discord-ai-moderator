...
class ErrorManager {
   ...
   */
  getRecoveryStrategy(error, source, context) {
    // Default strategy
  ...
  async checkDatabaseHealth() {
    ...
    try {
      ...
      // Update status
      this.serviceStatus.database.healthy = true;
  ...
  async enableDegradedMode() {
    ...
    try {
      ...
      if (this.serviceStatus.discord.healthy) {
        ...
        const configs = await ServerConfig.find({ enabled: true });
        ...
        for (const config of configs) {
          try {
            ...
            logger.error(`Error notifying guild ${config.serverId} about degraded mode:`, guildError);
          }
        }
      }
    } catch (error) {
      ...
      if (this.serviceStatus.discord.healthy) {
        ...
        for (const config of configs) {
          try {
            ...
            // Just log and continue to next guild
            logger.error(`Error notifying guild ${config.serverId} about service restoration:`, guildError);
          }
        }
      }
    } catch (error) {
