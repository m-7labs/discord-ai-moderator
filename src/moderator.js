...
const processMessage = async (message) => {
  ...
  try {
    ...
    if (!analysis || (!analysis.isViolation && !Object.prototype.hasOwnProperty.call(analysis, 'isViolation'))) {
 ...
 */
const logSkippedMessage = async (message, reason) => {
  try {
    await logAction({
      serverId: message.guild.id,
      userId: message.author.id,
 ...
 */
const logActionTaken = async (message, analysis, source) => {
  try {
    await logAction({
      serverId: message.guild.id,
      userId: message.author.id,
 ...
 * @param {Object} analysis - Analysis result
 */
const logNonViolation = async (message, analysis) => {
  try {
    await logAction({
      serverId: message.guild.id,
      userId: message.author.id,
